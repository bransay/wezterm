...
impl ShaderPathBuf {
    ...
    pub fn join_relative_to(&self, base: &Path) -> Self {
        match self {
            ShaderPathBuf::Native(p) => {
                if p.is_absolute() {
                    ShaderPathBuf::Native(p.clone())
                } else {
                    ShaderPathBuf::Native(base.join(p))
                }
            }
            ShaderPathBuf::Imported(p) => {
                ShaderPathBuf::Imported(p.join_relative_to(base))
            }
        }
    }
}
    ...
    fn from_dynamic(
        ...
        match value {
            ...
            Value::Object(obj) => {
                ...
                let path: PathBuf = PathBuf::from_dynamic(path_value, FromDynamicOptions::default())?;

                match format {
                    Value::String(f) if f == "Ghostty" => {
                        Ok(ShaderPathBuf::Imported(ImportedShaderPathBuf::Ghostty(
                            GhosttyPathBuf(path),
                        )))
                    }
