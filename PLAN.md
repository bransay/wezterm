...
- [x] **Phase 5: Single-Source-of-Truth Uniform Pipeline** (merges old Phase 5+6)
  - **Goal:** Make `PostProcessUniform` (the Rust struct) the single source of truth for all shader uniform code. Eliminate the manual lockstep between the host struct, the native WGSL preamble, the ghostty GLSL prefix/patch, and the naga IR struct manipulation. Consolidate all wezterm↔ghostty convention differences (coordinate-space Y-flip, type conversions, field naming) into the ghostty patch's `populate_globals()` rather than host-side hacks or IR surgery.
  - **Deliverable:** (1) Two new crates: `wezterm-shader-types` (plain crate: `UniformType` enum `Vec2/Float/UInt/Vec4` + `UniformField { name, ty }`) and `wezterm-shader-codegen` (proc-macro `#[derive(UniformBuffer)]` emitting `UNIFORM_FIELDS: &[UniformField]` reflection data — no target-language knowledge). (2) `PostProcessUniform` annotated with `#[uniform_type(Vec2)]` (required on every non-ignored field, error if missing) + `#[uniform_ignore]` (padding). (3) Native preamble: `POSTPROCESS_PREAMBLE` split into a runtime-rendered WGSL struct (from `UNIFORM_FIELDS`, cached in `OnceLock`) + static tail (vertex shader, bindings, `VertexOutput`). (4) Ghostty patch rewritten: `{{UNIFORM_BLOCK}}` placeholder + plain `struct Globals` (27 members, no longer a uniform block) + `Globals _g` + `#define iX _g.iX` aliases + `populate_globals()` (reads the wezterm uniform block, writes `_g`, includes the Y-flip for cursor rects) + `main()` calls `populate_globals()` first. `shader_import.rs` renders the GLSL uniform block from `UNIFORM_FIELDS` into the placeholder, cached. (5) Delete `replace_globals_struct` and `remap_globals_access_indices` from `shader_import.rs` — no struct replacement needed. (6) Revert `pane.rs` Y-flip hack (DD-023) — the transform function handles it. (7) `PostProcessUniform` uses wezterm-native field names/conventions throughout — no ghostty leakage into the host struct.
  - **Exit criteria:** Adding a uniform field requires editing only `PostProcessUniform` (add field + annotation); the WGSL preamble and GLSL uniform block are rendered from the reflected `UNIFORM_FIELDS`. The naga IR struct manipulation (`replace_globals_struct`/`remap_globals_access_indices`) is deleted. The `y + h` Y-flip hack is gone. `cursor_blaze.glsl`, `cursor_lightning.glsl`, `in-game-crt-cursor.glsl` still import and render correctly. All existing tests pass.
  - **Commit:** e9107dd3f