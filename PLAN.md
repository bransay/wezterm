...
- [ ] **Phase 5: Cursor Coordinate-Space Conversion** (deferred)
  - **Goal:** Replace the ad-hoc Y-flip at `pane.rs:115` (`y + h`) with a scalable, self-documenting conversion between wezterm's top-left-origin cursor rects and Ghostty's bottom-left-origin (Y-down) shader rects, so the convention is captured in one place and can't silently drift.
  - **Deliverable:** TBD — likely a named helper (e.g. `to_shader_rect`) or a type-enforced newtype for shader-space rects; decide during design. Must be reusable for any future cursor geometry, not just the current `iCurrentCursor` write.
  - **Exit criteria:** The Y-flip is expressed through a named, documented conversion (not an inline `+ h`); the blaze/lightning shaders still track the cursor correctly; existing tests pass.
  - **Commit:** <SHA filled in on completion>

- [ ] **Phase 6: Decouple Struct Definitions from Dual Maintenance** (deferred)
