...
## [2026-08-13 23:30] Phase 5: Design-focused implementation — reflection-based uniform synthesis
- **Did:** Design session for Phase 5 (single-source-of-truth uniform pipeline). Iterated with Bryan on the architecture. Key pivots: (1) rejected build.rs + syn (can't see field attributes without unstable `register_attr`); (2) rejected proc-macro emitting WGSL+GLSL strings directly (centralizes target-language knowledge in the codegen crate — not scalable); (3) settled on **reflection data + local rendering**: the proc-macro emits a `&[UniformField]` reflection array (names + `UniformType`), and each use site renders it into its own shader language at runtime, cached in `OnceLock`. Two crates: `wezterm-shader-types` (plain crate: `UniformType` enum `Vec2/Float/UInt/Vec4` + `UniformField` struct) and `wezterm-shader-codegen` (proc-macro `#[derive(UniformBuffer)]`, emits `UNIFORM_FIELDS`). Field annotations: `#[uniform_type(Vec2)]` (required on every non-ignored field, error if missing) + `#[uniform_ignore]` (padding). `#[ignore]` collided with a builtin attribute → renamed `#[uniform_ignore]`. The ghostty mapping (names + Y-flip + type conversions) lives in the patch's `populate_globals()` — the struct stays wezterm-native.
- **Files:** wezterm-shader-types/ (new), wezterm-shader-codegen/ (new), Cargo.toml (workspace members + deps), wezterm-gui/Cargo.toml
- **Refs:** PLAN.md#phase-5
- **Next:** Implement Phase 5

## [2026-08-14 06:30] Phase 5: Implementation complete — reflection synthesis + patch rewrite
- **Did:** Implemented Phase 5. (1) `wezterm-shader-types` crate: `UniformType` enum (Vec2/Float/UInt/Vec4) + `UniformField { name, ty }`. (2) `wezterm-shader-codegen` crate: `#[derive(UniformBuffer)]` proc-macro emitting `UNIFORM_FIELDS: &[UniformField]`; reads `#[uniform_type(...)]` / `#[uniform_ignore]`; errors on missing annotation. (3) `webgpu.rs`: `PostProcessUniform` annotated; `POSTPROCESS_PREAMBLE` split into `render_wgsl_uniform_struct(UNIFORM_FIELDS)` (cached in `postprocess_preamble()` OnceLock) + static `POSTPROCESS_PREAMBLE_TAIL`. (4) `shader_import.rs`: deleted `replace_globals_struct` + `remap_globals_access_indices` (naga IR surgery gone); added `render_glsl_uniform_block(UNIFORM_FIELDS)` + `ghostty_prefix()` (OnceLock) that renders the GLSL block into the patched template's `{{UNIFORM_BLOCK}}` placeholder. (5) Patch rewritten: `{{UNIFORM_BLOCK}}` placeholder + plain `struct Globals` (27 members, no longer a uniform block) + `Globals _g` + `#define iX _g.iX` aliases + `populate_globals()` (reads wezterm block, writes `_g`, includes Y-flip `y + w` for cursor rects) + `main()` calls `populate_globals()` first. (6) `pane.rs`: reverted Y-flip (`y + h` → `y`) — now handled in `populate_globals()`. All 47 wezterm-gui + 15 config tests pass. Patch regenerated via `diff -u` (hand-written hunks were malformed).
